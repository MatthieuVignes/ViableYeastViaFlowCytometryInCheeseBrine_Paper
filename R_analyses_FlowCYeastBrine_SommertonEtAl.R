# Data loading
data <- read.csv("20240613 Yeast brine AFU vs CFU study v3(pub).csv")
data2 <- read.csv("20231220S_cerevisiae_flow_rate_in_brine_PBS(live).csv")

setwd("Documents/Work-notMassey/FONTERRA/YeatInBrine_FlowC_Ben/")
...
dataFlowA <- data.frame(Log.Yeast = rep(dataFlowA$Log.Yeast.CFU.mL.or.Log.AFU.mL, 4), DaysF = c(rep(5, 12)), Exp = rep(dataFlowA$Exp, 4))
...
dataFlowB <- data.frame(Log.Yeast = rep(dataFlowB$Log.Yeast.CFU.mL.or.Log.AFU.mL, 4), DaysF = c(rep(5, 12)), Exp = rep(dataFlowB$Exp, 4))
...
dataFlowC <- data.frame(Log.Yeast = rep(dataFlowC$Log.Yeast.CFU.mL.or.Log.AFU.mL, 4), DaysF = c(rep(5, 12)), Exp = rep(dataFlowC$Exp, 4))
...
dataFlowD <- data.frame(Log.Yeast = rep(dataFlowD$Log.Yeast.CFU.mL.or.Log.AFU.mL, 4), DaysF = c(rep(5, 12)), Exp = rep(dataFlowD$Exp, 4))
...
dataFlowE <- data.frame(Log.Yeast = rep(dataFlowE$Log.Yeast.CFU.mL.or.Log.AFU.mL, 4), DaysF = c(rep(5, 12)), Exp = rep(dataFlowE$Exp, 4))
...
# Paper plot version
tiff(filename = "paperDay5Plate_vs_FCM(pub).tiff", width = 960, height = 3200)
...
MEAN.P <- matrix(NA, ncol = 5, nrow = 3)
MEAN.F <- matrix(NA, ncol = 5, nrow = 3)
pval.FmP <- matrix(NA, ncol = 5, nrow = 3)
SDwithinPlate <- matrix(NA, ncol = 5, nrow = 3) # 3(Exp)*5(Brine.sample)
...
for(i in 1:3){
  for(j in 1:5){
    ...
    MEAN.P[i,j] <- mean(dataPlate$Log.Yeast.CFU.mL.or.Log.AFU.mL[(dataPlate$Exp == i)&(dataPlate$Brine.sample == levels(dataPlate$Brine.sample)[j])])
    SDwithinPlate[i,j] <- sd(dataPlate$Log.Yeast.CFU.mL.or.Log.AFU.mL[(dataPlate$Exp == i)&(dataPlate$Brine.sample == levels(dataPlate$Brine.sample)[j])])
    xx <- dataFlow$Log.Yeast.CFU.mL.or.Log.AFU.mL[(dataFlow$Exp == i)&(dataFlow$Brine.sample == levels(dataFlow$Brine.sample)[j])]
    yy <- dataPlate$Log.Yeast.CFU.mL.or.Log.AFU.mL[(dataPlate$Exp == i)&(dataPlate$Brine.sample == levels(dataPlate$Brine.sample)[j])]
...
t(SDwithinFCM)
t(SDwithinPlate[,]) # at incub.day = 5 for Plate counts
